...
const KIT="https://cdn.jsdelivr.net/gh/workinwithai-create/PreEight@main/public/samples/drums";
...
const H={o:{midi:63,label:"conga open",src:`${SF}/steel_drums-mp3/C3.mp3`},
s:{midi:62,label:"conga slap",src:`${SF}/agogo-mp3/C5.mp3`},
m:{midi:64,label:"conga mute",src:`${SF}/steel_drums-mp3/G2.mp3`},
bh:{midi:60,label:"bongo macho",src:`${SF}/steel_drums-mp3/G4.mp3`},
bl:{midi:61,label:"bongo hembra",src:`${SF}/steel_drums-mp3/C4.mp3`},
c:{midi:75,label:"clave",src:`${SF}/agogo-mp3/G5.mp3`},
k:{midi:36,label:"kick",src:`${KIT}/kick.mp3`},
n:{midi:38,label:"snare",src:`${KIT}/snare.mp3`},
h:{midi:42,label:"hat",src:`${KIT}/hihat.mp3`}};
async function loadSample(hit){const key=hit.src;if(state.buffers.has(key))return state.buffers.get(key);const ctx=await ensureCtx();const res=await fetch(key);if(!res.ok)throw new Error("sample "+key);const buf=await ctx.decodeAudioData(await res.arrayBuffer());state.buffers.set(key,buf);return buf;}
...
async function seat(){setStatus("Seating live FluidR3 hands…");try{await Promise.all(Object.values(H).map(h=>loadSample(h)));setStatus("Chairs seated. Live FluidR3 steel drums + agogo.");}catch(e){setStatus("Some chairs missed a sample. Play anyway.");}}
...
function playHit(ctx,when,hit,gain=.85){const buf=state.buffers.get(hit.src);if(!buf)return;const src=ctx.createBufferSource();src.buffer=buf;const g=ctx.createGain();g.gain.value=gain;src.connect(g).connect(ctx.destination);src.start(when);state.sources.push(src);}
...
$("midi").onclick=()=>download(writeMidi(pocket()),"congafour-"+state.groove.id+".mid","audio/midi");
$("loop").onchange=async e=>{const file=e.target.files&&e.target.files[0];if(!file)return;const ctx=await ensureCtx();state.loopBuf=await ctx.decodeAudioData(await file.arrayBuffer());$("loopLabel").textContent=file.name;state.chairs.kit=false;render();};
